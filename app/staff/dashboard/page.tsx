...
export default function DashboardPage() {
  ...
  if (isCollege) {
    return {
      ...
      pending:  collegeRows.filter(r => ['Pending', 'For Validation'].includes(r.current_status)).length,
      claimed:  collegeRows.filter(r => r.current_status === 'Claimed').length,
    };
  } else {
    return {
      sf9:      shsRows.filter(r => r.form_type === 'RO-0005').length,
      sf10:     shsRows.filter(r => r.form_type === 'RO-0005').length,
      pending:  shsRows.filter(r => ['Pending', 'For Validation'].includes(r.current_status)).length,
      claimed:  shsRows.filter(r => r.current_status === 'Claimed').length,
      };
    }
  ...
  { label: 'Pending',          filter: 'pending',     count: collegeRows.filter(r => ['Pending','For Validation'].includes(r.current_status)).length },
  { label: 'Processing',       filter: 'processing',  count: collegeRows.filter(r => ['For Payment','Paid','Processing'].includes(r.current_status)).length },
  { label: 'For Release',      filter: 'forrelease',  count: collegeRows.filter(r => r.current_status === 'For Release').length },
  { label: 'Claimed',          filter: 'claimed',     count: collegeRows.filter(r => r.current_status === 'Claimed').length },
  ];
  ...
  const SHS_TABS = [
    ...
    { label: 'Pending',    filter: 'pending',    count: shsRows.filter(r => ['Pending','For Validation'].includes(r.current_status)).length },
    { label: 'Processing', filter: 'processing', count: shsRows.filter(r => ['For Payment','Paid','Processing'].includes(r.current_status)).length },
    { label: 'For Release', filter: 'forrelease', count: shsRows.filter(r => r.current_status === 'For Release').length },
    { label: 'Claimed',    filter: 'claimed',    count: shsRows.filter(r => r.current_status === 'Claimed').length },
  ];
  ...
  const filteredRows = useMemo(() => {
    ...
    else if (activeTab === 'forrelease')  rows = rows.filter(r => r.current_status === 'For Release');
      ...
      <div className="page-body">
        ...
        {isCollege ? (
          <div className="stat-grid stat-grid-4">
            ...
            <StatCard borderColor="#198754" num={stats.claimed} numColor="#198754" label="Released This Month"   bg="rgba(25,135,84,0.12)" icon={<svg viewBox="0 0 24 24" fill="none" stroke="#198754" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{width:20,height:20}}><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>} />
          </div>
        ) : (
          <div className="stat-grid stat-grid-4">
