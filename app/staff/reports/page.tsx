...
type ApiRequest = {
  ...
  status_logs?:        { status: string; timestamp: string }[];
};
...
// ── Constants ─────────────────────────────────────────────────────────────────
const COLORS = ['#114B9F','#001C43','#FFA323','#6D4DF5','#808EA1','#198754','#E50019','#646363','#FFA323'];

const ALL_STATUSES = [
  'Pending','For Validation','Invalid Request','For Clearance',
  'For Billing','For Payment','Paid','For Processing','For Printing',
  'For Release','Claimed','Pending Shredding','Shredded','Rejected',
];

const IN_PROGRESS_STATUSES = new Set([
  'For Validation','For Clearance','For Billing','For Payment',
  'Paid','For Processing','For Printing','For Release',
]);
  ...
  return (
    <div className="stat-card" style={{ padding: 16, borderBottom: `3px solid ${color}` }}>
      <div className="stat-top">
        <div>
          <div className="stat-num" style={{ fontSize: 24, color }}>{loading ? '—' : num}</div>
          <div className="stat-label" style={{ fontSize: 12 }}>{label}</div>
        </div>
        <div className="stat-icon" style={{ background: bg, width: 32, height: 32, fontSize: 14, color }}>{icon}</div>
      </div>
    </div>
  );
}

/** Compute average days from Paid → Claimed across completed requests */
function computeAvgDays(reqs: ApiRequest[]): number | null {
  const times: number[] = [];
  reqs.forEach(r => {
    if (!r.status_logs) return;
    const paidLog    = r.status_logs.find(l => l.status === 'Paid');
    const claimedLog = r.status_logs.find(l => l.status === 'Claimed');
    if (!paidLog || !claimedLog) return;
    const days = (new Date(claimedLog.timestamp).getTime() - new Date(paidLog.timestamp).getTime())
      / (1000 * 60 * 60 * 24);
...
export default function ReportsPage() {
    ...
    async function fetchAll() {
      ...
      try {
        const [reqRes, payRes] = await Promise.all([
          fetch(`${API_BASE}/requests/?page_size=500`),
          fetch(`${API_BASE}/payments/?page_size=500`),
        ]);
        if (!reqRes.ok || !payRes.ok) throw new Error();
  ...
  const availableYears = useMemo(() => {
    ...
    requests.forEach(r => { if (r.date_submitted) years.add(new Date(r.date_submitted).getFullYear()); });
    return Array.from(years).sort((a, b) => b - a);
  }, [requests]);

  // ── Available months for selected year (dynamic) ──────────────────────────
  ...
  // ── Available months for export picker (all data) ─────────────────────────
  const availableMonthsAll = useMemo(() => {
    ...
    sorted.forEach(r => {
      ...
      if (!seen.has(value)) {
        seen.add(value);
        result.push({ value, label: d.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) });
      }
    });
    return result;
  }, [requests]);

  // ── Filter requests by selected year+month ────────────────────────────────
  ...
  const stats = useMemo(() => {
    ...
    const claimed    = filteredRequests.filter(r => r.current_status === 'Claimed').length;
    const inProgress = filteredRequests.filter(r => IN_PROGRESS_STATUSES.has(r.current_status)).length;
    ...
    const avgDays    = computeAvgDays(filteredRequests);
    const completionRate = total > 0 ? Math.round(claimed / total * 100) : 0;
    return { total, claimed, inProgress, college, shs, avgDays, completionRate };
  }, [filteredRequests]);
  ...
  const statusBreakdown = useMemo(() => {
    ...
    // Order by canonical status order, then any unknown ones at the end
    const ordered: [string, number][] = [];
    ALL_STATUSES.forEach(s => { if (counts[s]) ordered.push([s, counts[s]]); });
    Object.entries(counts).forEach(([s, c]) => {
      if (!ALL_STATUSES.includes(s)) ordered.push([s, c]);
    });
    return ordered;
  }, [filteredRequests]);
  ...
  const modeBreakdown = useMemo(() => ([
    { label: 'Online', count: filteredRequests.filter(r => r.submission_mode === 'Online').length, color: '#198754' },
    { label: 'Onsite', count: filteredRequests.filter(r => r.submission_mode === 'Onsite').length, color: '#FFA323' },
  ]), [filteredRequests]);

  // ── Monthly volume (last 6 months, always all-time for context) ───────────
  ...
  // ── PDF generator ─────────────────────────────────────────────────────────
  function generatePDF(exportReqs: ApiRequest[], exportPays: ApiPayment[], label: string) {
    const total      = exportReqs.length;
    const claimed    = exportReqs.filter(r => r.current_status === 'Claimed').length;
    const inProgress = exportReqs.filter(r => IN_PROGRESS_STATUSES.has(r.current_status)).length;
    ...
    const avgDays    = computeAvgDays(exportReqs);
    const completionRate = total > 0 ? Math.round(claimed / total * 100) : 0;

    const statusCounts: Record<string, number> = {};
    exportReqs.forEach(r => { statusCounts[r.current_status] = (statusCounts[r.current_status] || 0) + 1; });
    const statusRows = ALL_STATUSES
      .filter(s => statusCounts[s])
      .map(s => `<tr><td>${s}</td><td style="text-align:right;font-weight:700">${statusCounts[s]}</td><td style="text-align:right">${total > 0 ? Math.round(statusCounts[s]/total*100) : 0}%</td></tr>`)
      .join('');
    ...
    const monthRows = Object.entries(monthMap)
      ...
      .map(([m, c]) => `<tr><td>${m}</td><td style="text-align:right;font-weight:700">${c}</td></tr>`)
      .join('');

    const modeRows = [
      { label: 'Online', count: exportReqs.filter(r => r.submission_mode === 'Online').length },
      { label: 'Onsite', count: exportReqs.filter(r => r.submission_mode === 'Onsite').length },
    ].map(m => `<tr><td>${m.label}</td><td style="text-align:right;font-weight:700">${m.count}</td><td style="text-align:right">${total > 0 ? Math.round(m.count/total*100) : 0}%</td></tr>`).join('');

    const css = `*{box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:12px;margin:0;padding:24px;color:#000}.page{max-width:780px;margin:0 auto}.header{display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid #001C43;padding-bottom:12px;margin-bottom:20px}.school{font-size:16px;font-weight:900;color:#001C43}.sub{font-size:11px;color:#666;margin-top:2px}.report-title{font-size:20px;font-weight:900;color:#001C43;text-align:right}.period{font-size:12px;color:#666;text-align:right}.kpi-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:24px}.kpi{border:1px solid #ddd;border-radius:8px;padding:14px}.kpi-num{font-size:24px;font-weight:900;color:#001C43}.kpi-label{font-size:11px;color:#666;margin-top:2px}.section{margin-bottom:24px}.section-title{font-size:13px;font-weight:700;color:#001C43;border-bottom:1px solid #ddd;padding-bottom:6px;margin-bottom:12px;text-transform:uppercase;letter-spacing:.5px}.two-col{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#001C43;color:white;padding:7px 10px;text-align:left}td{padding:7px 10px;border-bottom:1px solid #eee}tr:nth-child(even) td{background:#f9f9f9}.footer{margin-top:32px;font-size:10px;color:#999;text-align:center;border-top:1px solid #eee;padding-top:12px}.completion-bar{height:12px;background:#eee;border-radius:6px;margin:8px 0}.completion-fill{height:100%;background:#198754;border-radius:6px}@media print{body{padding:10px}@page{margin:12mm}}`;

    const html = `<!DOCTYPE html><html><head><title>DRMS Report — ${label}</title><style>${css}</style></head><body><div class="page">
<div class="header">
  <div><div class="school">MAPÚA MALAYAN COLLEGES MINDANAO</div><div class="sub">Registrar's Office — Document Request Monitoring System</div></div>
  <div><div class="report-title">Reports &amp; Analytics</div><div class="period">Period: ${label}</div><div class="period">Generated: ${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div></div>
</div>
<div class="kpi-grid">
  <div class="kpi"><div class="kpi-num">${total}</div><div class="kpi-label">Total Requests</div></div>
  <div class="kpi"><div class="kpi-num" style="color:#198754">${claimed}</div><div class="kpi-label">Claimed</div></div>
  <div class="kpi"><div class="kpi-num" style="color:#FFA323">${inProgress}</div><div class="kpi-label">In Progress</div></div>
  <div class="kpi"><div class="kpi-num" style="color:#114B9F">${college}</div><div class="kpi-label">College</div></div>
  <div class="kpi"><div class="kpi-num" style="color:#6D4DF5">${shs}</div><div class="kpi-label">SHS</div></div>
</div>
<div class="two-col">
  <div class="kpi" style="display:flex;flex-direction:column;justify-content:center;">
    <div class="kpi-label">Completion Rate</div>
    <div class="kpi-num" style="color:#198754;font-size:32px;">${completionRate}%</div>
    <div class="completion-bar"><div class="completion-fill" style="width:${completionRate}%"></div></div>
    <div style="font-size:11px;color:#666">${claimed} of ${total} requests claimed</div>
  </div>
  <div class="kpi" style="display:flex;flex-direction:column;justify-content:center;">
    <div class="kpi-label">Avg. Days to Complete</div>
    <div class="kpi-num" style="color:#114B9F;font-size:32px;">${avgDays !== null ? avgDays : '—'}</div>
    <div style="font-size:11px;color:#666">${avgDays !== null ? 'days from payment to claim' : 'No completed requests yet'}</div>
  </div>
</div>
<div class="two-col">
  ...
  <div class="section"><div class="section-title">By Submission Mode</div><table><thead><tr><th>Mode</th><th>Count</th><th>Share</th></tr></thead><tbody>${modeRows}</tbody></table></div>
</div>
...
<div class="footer">DRMS — Registrar's Office, MMCM &nbsp;|&nbsp; System-generated report for internal use only.</div>
  ...
  function handleConfirmExport() {
    if (exportMode === 'all') {
      generatePDF(requests, payments, 'All Time');
    } else if (exportMode === 'current') {
      generatePDF(filteredRequests, filteredPayments, periodLabel);
    } else {
      const reqs = requests.filter(r => {
        if (!r.date_submitted) return false;
        const d = new Date(r.date_submitted);
        return pickedMonths.has(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`);
      });
      ...
      const label = pickedMonths.size === 1
        ? (availableMonthsAll.find(m => pickedMonths.has(m.value))?.label ?? 'Selected')
        : `${pickedMonths.size} months selected`;
      generatePDF(reqs, pays, label);
    }
    ...
    <>
      ...
      <div className="page-body">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--mid-gray)' }}>Viewing:</div>
          ...
          <select
            ...
            style={{ fontSize: 12, height: 34, border: '1px solid var(--border-col)', borderRadius: 6, padding: '0 10px', background: 'var(--surface)', color: 'var(--text-primary)', cursor: 'pointer', minWidth: 120 }}
          >
            <option value=''>All Years</option>
            {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
          </select>

          {/* Month dropdown — only shows when a year is selected */}
          {selectedYear !== null && (
            <select
              value={selectedMonth ?? ''}
              onChange={e => setSelectedMonth(e.target.value === '' ? null : parseInt(e.target.value))}
              style={{ fontSize: 12, height: 34, border: '1px solid var(--border-col)', borderRadius: 6, padding: '0 10px', background: 'var(--surface)', color: 'var(--text-primary)', cursor: 'pointer', minWidth: 140 }}
            >
              <option value=''>All Months</option>
              {availableMonthsForYear.map(m => <option key={m} value={m}>{MONTHS[m]}</option>)}
            </select>
          )}

          {/* Active filter pill */}
          {(selectedYear !== null) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(17,75,159,0.08)', borderRadius: 20, padding: '4px 10px' }}>
              <span style={{ fontSize: 12, fontWeight: 600, color: '#114B9F' }}>{periodLabel}</span>
              <button
                onClick={() => { setSelectedYear(null); setSelectedMonth(null); }}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#114B9F', fontSize: 14, lineHeight: 1, padding: 0 }}
              >×</button>
            </div>
          )}
        ...
        {/* ── Stat cards ── */}
        <div className="stat-grid stat-grid-5" style={{ marginBottom: 20 }}>
          <StatCard loading={loading} num={stats.total}      label="Total Requests"  color="#001C43" bg="rgba(0,28,67,0.1)"     icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{width:18,height:18}}><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>} />
          ...
          <StatCard loading={loading} num={stats.college}    label="College"         color="#114B9F" bg="rgba(17,75,159,0.12)"  icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{width:18,height:18}}><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c0 2 2 3 6 3s6-1 6-3v-5"/></svg>} />
          <StatCard loading={loading} num={stats.shs}        label="SHS"             color="#6D4DF5" bg="rgba(109,77,245,0.12)" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{width:18,height:18}}><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg>} />
        </div>

        {/* ── Completion rate + Avg days row ── */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20, marginBottom: 20 }}>

          {/* Completion Rate */}
          <div className="drms-card" style={{ padding: 20 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--mid-gray)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Completion Rate</div>
            <div style={{ fontSize: 32, fontWeight: 800, color: '#198754' }}>
              {loading ? '—' : `${stats.completionRate}%`}
            </div>
            <div style={{ height: 8, background: 'var(--surface-2)', borderRadius: 4, margin: '10px 0 6px' }}>
              <div style={{ height: '100%', width: `${stats.completionRate}%`, background: '#198754', borderRadius: 4, transition: 'width .3s' }} />
            </div>
            <div style={{ fontSize: 12, color: 'var(--mid-gray)' }}>{stats.claimed} of {stats.total} claimed</div>
          </div>

          {/* Avg Days to Complete */}
          <div className="drms-card" style={{ padding: 20 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--mid-gray)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Avg. Days to Complete</div>
            <div style={{ fontSize: 32, fontWeight: 800, color: '#114B9F' }}>
              {loading ? '—' : stats.avgDays !== null ? stats.avgDays : '—'}
            </div>
            <div style={{ fontSize: 12, color: 'var(--mid-gray)', marginTop: 8 }}>
              {stats.avgDays !== null ? 'days from payment to claim' : 'No completed requests yet'}
            </div>
          </div>

          {/* Submission mode */}
          <div className="drms-card" style={{ padding: 20 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--mid-gray)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 14 }}>By Submission Mode</div>
            {modeBreakdown.map(m => (
              <div key={m.label} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <div style={{ flex: 1, fontSize: 12, color: 'var(--text-primary)' }}>{m.label}</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: m.color, minWidth: 24, textAlign: 'right' }}>{loading ? '—' : m.count}</div>
                <div style={{ width: 80, height: 6, background: 'var(--surface-2)', borderRadius: 3 }}>
                  <div style={{ height: '100%', width: stats.total > 0 ? `${Math.round(m.count/stats.total*100)}%` : '0%', background: m.color, borderRadius: 3 }} />
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ── Form type + Status breakdown ── */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>

          {/* By Form Type */}
          <div className="drms-card" style={{ padding: 20 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--mid-gray)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 14 }}>By Form Type</div>
            {formBreakdown.map(f => (
              <div key={f.label} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: f.color, minWidth: 24, textAlign: 'right' }}>{loading ? '—' : f.count}</div>
          ...
          {/* By Status */}
          <div className="drms-card" style={{ padding: 20 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--mid-gray)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 14 }}>By Status</div>
            {loading ? (
        ...
        {/* ── Monthly volume chart + Academic level ── */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div className="drms-card" style={{ padding: 20 }}>
            <div style={{ paddingBottom: 14, borderBottom: '1px solid rgba(0,0,0,0.06)', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--mid-gray)', marginTop: 3 }}>Always shows all-time data for trend context</div>
            </div>
            {loading ? (
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, height: 160, paddingBottom: 24, position: 'relative' }}>
                {monthlyVolume.map(([month, count]) => (
                  <div key={month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    ...
                    <div style={{ width: '100%', height: count > 0 ? `${Math.round(count / maxMonthly * 120)}px` : '4px', background: count > 0 ? '#114B9F' : 'var(--surface-2)', borderRadius: '4px 4px 0 0', transition: 'height .3s', minHeight: 4 }} />
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary)' }}>Export Report as PDF</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
              ...
              {/* All Time */}
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', borderRadius: 10, border: `1.5px solid ${exportMode === 'all' ? '#114B9F' : 'rgba(0,0,0,0.1)'}`, background: exportMode === 'all' ? 'rgba(17,75,159,0.04)' : 'transparent', cursor: 'pointer' }}>
              ...
              {/* Current view */}
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', borderRadius: 10, border: `1.5px solid ${exportMode === 'current' ? '#114B9F' : 'rgba(0,0,0,0.1)'}`, background: exportMode === 'current' ? 'rgba(17,75,159,0.04)' : 'transparent', cursor: 'pointer' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--mid-gray)', marginTop: 2 }}>
                    Export {filteredRequests.length} request{filteredRequests.length !== 1 ? 's' : ''} shown on screen
                  </div>
                </div>
              </label>

              {/* Pick specific months */}
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', borderRadius: 10, border: `1.5px solid ${exportMode === 'pick' ? '#114B9F' : 'rgba(0,0,0,0.1)'}`, background: exportMode === 'pick' ? 'rgba(17,75,159,0.04)' : 'transparent', cursor: 'pointer' }}>
                    ...
                    <>
                      <div style={{ display: 'flex', gap: 10, marginBottom: 4 }}>
                        <button style={{ fontSize: 11, color: 'var(--blue)', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
                          onClick={() => setPickedMonths(new Set(availableMonthsAll.map(m => m.value)))}>Select all</button>
                        ...
                        <button style={{ fontSize: 11, color: 'var(--mid-gray)', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
                          onClick={() => setPickedMonths(new Set())}>Clear</button>
                      </div>
                      {availableMonthsAll.map(m => (
                        <label key={m.value} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-primary)', cursor: 'pointer', padding: '4px 0' }}>
                          <input type="checkbox" checked={pickedMonths.has(m.value)} onChange={() => togglePickedMonth(m.value)} style={{ accentColor: '#114B9F', width: 14, height: 14 }} />
                          {m.label}
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 'auto' }}>
              ...
              <button className="btn-outline" style={{ padding: '0 18px' }} onClick={() => setExportModalOpen(false)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
